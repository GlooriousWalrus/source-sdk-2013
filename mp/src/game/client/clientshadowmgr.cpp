...
bool CClientShadowMgr::Init()
{
	...
	//enable multiple shadow maps
	//bool bTools = CommandLine()->CheckParm( "-tools" ) != NULL;
	m_nMaxDepthTextureShadows = 8;	//  8 ---Just one shadow depth texture in games, more in tools
...
void CClientShadowMgr::BuildFlashlight( ClientShadowHandle_t handle )
{
	...
	// Don't project the flashlight if the frustum AABB is not in our view	Vector mins, maxs;	CalculateAABBFromProjectionMatrix(shadow.m_WorldToShadow, &mins, &maxs); 	if(engine->CullBox(mins, maxs))		return;
